...
void GameWindow::Initialise(SDL_Renderer **renderer, SDL_Window **window/*, SDL_Surface **screen*/) {
    if(SDL_Init(SDL_INIT_EVERYTHING) == -1)
        mLog.sdl_bomb("Failed to Initialise SDL");
    ...
    *window = SDL_CreateWindow(
    ...
    if(*window == nullptr)
        mLog.sdl_bomb("Failed to create SDL Window");

    *renderer = SDL_CreateRenderer(*window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    if(*renderer == nullptr)
        mLog.sdl_bomb("Failed to create SDL Renderer");

    const int flags = IMG_INIT_PNG | IMG_INIT_JPG;
    if(IMG_Init(flags) !=flags)
        mLog.sdl_bomb("Failed to load the Image loading extensions");

    if(TTF_Init() != 0)
        mLog.sdl_bomb("Failed to load TTF extension");
